import { Allocation, Company, Difficulty, Financials, GameEvent, GameState, Industry, TurnReport, DEFAULT_EFFECTS } from './types'
...
export function processTurn(
  ...
  const { company, financials, turn, marketSentiment, difficulty, activeEffects } = state
  ...
  // ── アイテム効果を加味 ──
  const fx = activeEffects ?? DEFAULT_EFFECTS

  // 🛡️ 危機管理マニュアル: ネガティブイベントを無効化
  let event = pickEvent(difficulty, allocation, budget)
  const nullified = fx.nullifyNextNegEvent && event && isNegativeEvent(event)
  if (nullified) event = null

  let revenueMultiplier = growth * rdBonus * mktBonus * hireBonus * capexBonus
  let expenseMultiplier = baseCostGrowth

  // 🚀 ロケットブースター: 売上倍率
  if (fx.revenueBoostTurns > 0) {
    revenueMultiplier *= fx.revenueMultiplier
  }
  ...
  // 📋 方針改正書: 利益倍率
  const rawProfit = newRevenue - newExpenses
  const newProfit = Math.round(rawProfit * (fx.profitMultiplier ?? 1))
  ...
  let newSentiment = Math.max(-0.6, Math.min(0.5,
    marketSentiment * decayRate + eventSentiment + (Math.random() - negBias) * 0.08,
  ))

  // ⚡ 神の一手: センチメントを強制プラスに
  if (fx.forcePositiveSentiment) {
    newSentiment = Math.max(newSentiment, 0.25 + Math.random() * 0.15)
  }
  ...
  const nextEffects = {
    ...
    nullifyNextNegEvent: nullified ? false : fx.nullifyNextNegEvent, // 発動したらリセット
    revenueBoostTurns: Math.max(0, fx.revenueBoostTurns - 1),
    revenueMultiplier: fx.revenueBoostTurns <= 1 ? 1 : fx.revenueMultiplier,
  }
...
function isNegativeEvent(event: GameEvent): boolean {
  const neg = event.effect.revenueMultiplier !== undefined && event.effect.revenueMultiplier < 1
  const neg2 = event.effect.stockSentiment !== undefined && event.effect.stockSentiment < 0
  return neg || neg2
}
