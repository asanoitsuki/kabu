...
export default function StockChart({ compact = false }: Props) {
  ...
  return (
    <div className={`bg-gray-900 rounded-2xl ${compact ? 'p-3' : 'p-5'}`}>
      <div className={`flex items-center justify-between ${compact ? 'mb-2' : 'mb-4'}`}>
        <div>
          <div className={`font-black text-white ${compact ? 'text-lg' : 'text-3xl'}`}>
            ¥{last.toLocaleString()}
          </div>
          <div className={`font-semibold mt-0.5 ${compact ? 'text-xs' : 'text-sm'} ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
