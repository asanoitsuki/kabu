...
export default function GameOverScreen({ onShowHistory, onShowRanking }: Props) {
  ...
  useEffect(() => {
    ...
    // ガチャコイン付与
    const gradeCoins = grade === 'S' ? 3 : grade === 'A' ? 2 : grade === 'B' ? 1 : 0
    if (gradeCoins > 0) useItemStore.getState().addCoins(gradeCoins)
    setCoinsGained(gradeCoins)
    const newTotalXp = useAchievementStore.getState().xp
    useItemStore.getState().syncXpCoins(newTotalXp)
        ...
        {xpGained > 0 && (
          <div className="bg-indigo-950 border border-indigo-800 rounded-2xl p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              {coinsGained > 0 && (
                <div className="flex items-center gap-1 bg-yellow-950 border border-yellow-800 rounded-xl px-3 py-1.5">
                  <span className="text-yellow-400">🪙</span>
                  <span className="text-yellow-300 font-black text-sm">+{coinsGained}</span>
                </div>
              )}
            </div>
            {xpRequired > 0 && (
